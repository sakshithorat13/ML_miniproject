...
def run(age: Optional[int] = None):
    ...
    # --- Insights Section (Improved and Detailed) ---
    insights_table = [
        {
            
            "Graph": "🧩 SVM Decision Boundary (Training vs Test)",
"Insight": (
    "<b>Overview:</b> The SVM model separates patients with and without heart disease using "
    "<b>Age</b> and <b>Cholesterol</b> levels. The solid black line represents the decision boundary, "
    "while the dashed lines show the margins that define how confidently each class is separated. "
    "Hollow points indicate support vectors — the most critical cases influencing the boundary.<br><br>"

    "<b>Key Observations:</b><ul>"
    "<li>The <b>red</b> and <b>blue</b> regions represent predicted heart disease and no-disease zones respectively.</li>"
    "<li>Most data points fall correctly within their class regions, showing <b>good model generalization</b>.</li>"
    "<li>Borderline samples near the central line represent <b>uncertain or overlapping cases</b> where predictions are less confident.</li>"
    "<li>The similarity between training and test plots shows the model has learned stable and meaningful patterns.</li>"
    "<li><b>Support vectors</b> mark patients near the risk threshold, crucial for defining the boundary width.</li>"
    "</ul>"

    "<b>Interpretation:</b><ul>"
    f"<li><b>Accuracy:</b> {acc*100:.2f}% | <b>Precision:</b> {prec*100:.2f}% | "
    f"<b>Recall:</b> {rec*100:.2f}% | <b>F1-Score:</b> {f1*100:.2f}%</li>"
    f"<li><b>Support Vectors:</b> {len(svm.support_vectors_)}</li>"
    "<li>The model successfully captures how increasing <b>age</b> and <b>cholesterol</b> levels elevate heart disease risk.</li>"
    "<li>The clear, linear boundary indicates a <b>strong and interpretable relationship</b> between the two features.</li>"
    "<li>Adding other medical factors like <b>blood pressure</b> or <b>ECG data</b> could further improve predictive accuracy.</li>"
    "<li>Overall, the SVM provides a balanced and transparent classifier that performs well for simple, two-feature prediction.</li>"
    "</ul>"
)
